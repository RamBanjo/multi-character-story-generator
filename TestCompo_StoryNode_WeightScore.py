...
#Ah, I knew it wouldn't be as easy as I initially thought~ There just happens to be the matter of the absolute step where it would take place, so we also need to take care of that.
#New idea: use the moving world states to test for time-specificity for each rule. Like this:
# 1. Rule A, Abs Step 2 (Score: 5)
# 2. Rule A, Abs Step 5 (Score: 3)
# 3. Rule A, Abs Step 12 (Score: 2)
#That works for one node, but remember, character's state changes inbetween world states, so we need to account for that as well. If we're testing for the score of a sequence of story nodes, we can assume that character will change according to the sequences of that node.
#For example:
#
