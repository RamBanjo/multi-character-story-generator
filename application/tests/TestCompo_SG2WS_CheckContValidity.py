...
from application.components.ConditionTest import HasEdgeTest, SameLocationTest, HeldItemTagTest
from application.components.RelChange import RelChange
...
go_to_alicehouse = StoryNode("Go to Alicehouse", None, {"Type": "movement"}, 1, effects_on_next_ws=[move_to_alicehouse, move_away], required_test_list=[double_edge_alicehouse])
...
double_edge_park = HasEdgeTest(GenericObjectNode.GENERIC_LOCATION, "adjacent_to", park, two_way=True)

go_park = StoryNode("Go to Park", None, {"Type": "movement"}, 1, effects_on_next_ws=[move_away, move_to_park], required_test_list=[double_edge_park])
...
talk_charlie = StoryNode("Talk to Charlie about Present", None, {"Type": "conversation"}, 2, required_test_list=[bob_not_hate_charlie, bob_has_present])
...
give_present_to_alice = StoryNode("Give Present to Alice", None, {"Type": "give_item"}, 2, effects_on_next_ws=[], required_test_list=[bob_holds_present, bob_likes_alice, bob_same_room_alice])
