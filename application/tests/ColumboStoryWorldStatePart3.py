...
# columbo = CharacterNode(name="Columbo", biases={"lawbias":0, "moralbias":0}, tags={"Type":"Character","Job":"Explorer","Species":"Human", "Goal":"Explore New World","Alive":False, "PlotArmor":False, "Stranded":True}, internal_id=0)
...
greenland_insects = ObjectNode(name="Greenland Insects", tags={"Type":"Mob", "Count":0, "Behavior":"Aggressive", "EdibleFlesh":True, "Faction":None},internal_id=11)
...
enemy_mercenary = ObjectNode(name="Enemy Mercenary", tags={"Type":"Mob","Count":0, "Behavior":"Aggressive", "Faction":"Mercenary"}, internal_id=14)
earth_army = ObjectNode(name="Earth Army", tags={"Type":"Mob","Count":2, "Behavior":"Passive", "Faction":"Earth"}, internal_id=15)

all_characters = [iris, amil, apollo, alien_god]
...
world_state.connect(from_node=new_world_greenland, edge_name="holds", to_node=amil)
...
world_state.connect(from_node=new_world_greenland, edge_name="holds", to_node=earth_army)
...
world_state.connect(from_node=amil, edge_name="KillReason", to_node=iris, value="Avenge")
...
actor_gains_kill_relationship = RelChange(name="Actor is the Killer of Target", node_a=GenericObjectNode.GENERIC_ACTOR, edge_name="killer_of", node_b=GenericObjectNode.GENERIC_TARGET, add_or_remove=ChangeAction.ADD)

kill_another_actor = StoryNode(name="Actor Kills Actor", tags={"Type":"Murder"}, charcount=1, target_count=1, effects_on_next_ws=[target_becomes_dead, actor_gains_kill_relationship], required_test_list=[target_is_alive, actor_has_reason_to_kill_target, target_no_plot_armor, actor_shares_location_with_target])

# Columbo Death Kill Reason Activate:
actor_is_human = HasTagTest(object_to_test=GenericObjectNode.GENERIC_ACTOR, tag="Species", value="Human")
target_is_human = HasTagTest(object_to_test=GenericObjectNode.GENERIC_TARGET, tag="Species", value="Human")

something_killed_target = HasEdgeTest(object_from_test=GenericObjectNode.CONDITION_TESTOBJECT_PLACEHOLDER, edge_name_test="killer_of", object_to_test=GenericObjectNode.GENERIC_TARGET, soft_equal=True)
actor_kill_reason_something = RelChange(name="Gain Avenge Kill Reason", node_a=GenericObjectNode.GENERIC_ACTOR, edge_name="KillReason", node_b=GenericObjectNode.CONDITION_TESTOBJECT_PLACEHOLDER, value="Avenge", add_or_remove=ChangeAction.ADD)
kill_reason_towards_killer_change = ConditionalChange(name="If Killer then Kill Reason", list_of_condition_tests=[something_killed_target], list_of_changes=[actor_kill_reason_something])

avenge_human = StoryNode(name = "Avenge Human", biasweight=100, charcount=1, target_count=1, effects_on_next_ws=[kill_reason_towards_killer_change], required_test_list=[actor_is_alive, actor_is_human, target_is_human, target_dead])
...
attack_inhabitants = StoryNode(name="Attack Inhabitants", tags={"Type":"Fight"}, required_test_list=[actor_is_alive, actors_goal_is_eradication, actor_shares_location_with_target, target_is_mob, target_count_greater_than_0])
...
massacre_mob = StoryNode(name="Massacre Mobs", tags={"Type":"Destruction"}, effects_on_next_ws=[target_count_zeros], required_test_list=[actor_is_alive, actor_shares_location_with_target, actors_goal_is_eradication, target_count_greater_than_0, target_is_mob])      
...
actor_gets_kill_reason_towards_target_invasion = RelChange(name="Gain Invasion Kill Reason", node_a=GenericObjectNode.GENERIC_ACTOR, edge_name="KillReason", node_b=GenericObjectNode.GENERIC_TARGET, value="PlanetInvader", add_or_remove=ChangeAction.ADD)
target_loses_plot_armor = TagChange(name="Target Loses Plot Armor", object_node_name=GenericObjectNode.GENERIC_TARGET, tag="PlotArmor", value=False, add_or_remove=ChangeAction.ADD)

notice_invader = StoryNode(name="Notice Planet Invader", charcount=1, target_count=1, effects_on_next_ws=[actor_gets_kill_reason_towards_target_invasion, target_loses_plot_armor], required_test_list=[actor_is_alive, target_is_alive, actor_shares_location_with_target, target_is_human], biasweight=200)
...
command_army_attack = StoryNode(name="Command Army to Attack", target_count=1, effects_on_next_ws=[if_commanded_by_actor_then_kill_target_reason], required_test_list=[actor_is_alive, actor_has_reason_to_kill_target, actor_commands_a_mob_with_greater_count, target_is_alive])
...
record_earth_army_data = StoryNode(name="Record Earth Army Data", required_test_list=[actor_is_alive, actor_shares_location_with_mob_from_earth, actor_not_have_earth_army_data], effects_on_next_ws=[actor_gains_earth_army_data], biasweight=100)
...
recall_tasks = StoryNode(name="Iris Remembers Tasks", effects_on_next_ws=[get_attack_tatain_taskchange, get_attacked_by_robots_taskchange], required_test_list=[actor_is_alive], charcount=1, target_count=1)
# Invade Greenland
# Source: Alien God
# Owner: Apollo
...
start_avenge_human = JoiningJointRule(rule_name="Start Avenge Human", base_actions=[], joint_node=avenge_human)
# ContJointRule
...
story_graph.insert_joint_node(joint_node=recall_tasks, main_actor=iris, targets=[alien_god], location=new_world_greenland, absolute_step=0, timestep=0)
story_graph.insert_story_part(part=DEFAULT_WAIT_NODE, character=amil, location=new_world_greenland, absolute_step=0, timestep=0)
...
# for item in stacks:
#     print(story_graph.test_task_stack_advance_validity(task_stack_name=item, actor_name="Iris", abs_step=1))

# print(story_graph.test_task_stack_advance_validity(task_stack_name="Get Attacked by Bots Stack", actor_name="Iris", abs_step=1))

# ======= BELOW IS TESTING AREA AND DOES NOT CONTRIBUTE TO ANYTHING ABOVE =======

# story_graph.insert_story_part(part=DEFAULT_WAIT_NODE, character=iris, location=alien_god_planet, absolute_step=1, timestep=0)
# story_graph.insert_story_part(part=DEFAULT_WAIT_NODE, character=columbo, location=alien_god_planet, absolute_step=1, timestep=0)
# story_graph.insert_joint_node(joint_node=kill_another_actor, main_actor=iris, targets=[columbo], location=new_world_greenland, absolute_step=2, timestep=0)
# print(attempt_apply_rule(rule_object=start_kill_actor, perform_index=3, target_story_graph=story_graph, character_object=columbo, shortest_path_charname_list=["Columbo", "Iris"]))

# print("Abridged Story Nodes")
# print("==COLUMBO==")
# story_graph.print_all_nodes_from_characters_storyline(columbo)
# print("==IRIS==")
# story_graph.print_all_nodes_from_characters_storyline(iris)
