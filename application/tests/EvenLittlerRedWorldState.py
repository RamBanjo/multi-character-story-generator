...
current_location_has_someone_actor_fears = SomethingPassesAllGivenTestsTest(list_of_tests_with_placeholder=[current_location_holds_someone, actor_fears_someone], score=-10)
...
target_location_has_someone_actor_fears = SomethingPassesAllGivenTestsTest(list_of_tests_with_placeholder=[target_holds_someone, actor_fears_someone], score=10)
...
# TODO: Ask professor if we can shrink the world state. It's getting too big for my comfort.
# Things that we can change/remove without consequences:
# 1. Remove the plotline about Brick escaping to depend on Wood, he escapes to Red House instead
# 2. Remove the "path" locations.
# 3. Remove the individual houses. For residents of the Forest Village each character's house is listed in the location's tag instead.
# 4. Remove the Hunter. Red's Mom will hire the Papa Bear to kill the wolf instead.